  ...
  return {
    createSetter: function (key) {
      ...
      return function (value) {
        ...
        subscribes[cacheInd] = subscribes[cacheInd].reduce(function (
          r,
          resolve
        ) {
          resolve(value);
...
function validateArguments(args) {
  ...
  if (!detectSourceType(args[0])) {
    return new TypeError(
      "First argument should be HTMLImageElement, HTMLVideoElement, HTMLCanvasElement, File or String"
    );
  }
...
function downscale(source, destWidth, destHeight, options) {
  ...
  if (cache.has(source)) {
    ...
    cache.get(source, function (cacheData) {
      ...
      if (
        dims.sourceWidth / dims.destWidth >= 2 &&
        dims.sourceHeight / dims.destHeight >= 2
      ) {
        timing.mark();
        var destImageData = downsample(
          imageData,
          dims.destWidth,
          dims.destHeight,
          dims.sourceX,
  ...
  switch (detectSourceType(source)) {
    ...
    case "HTMLCanvasElement":
      timing.mark();
      scaleSourceResolve(source, source.width, source.height);
      break;
